...
The same document may also have a top-level `parameterGroups` array. Use
`parameterGroups` when you want to translate a water-data concept across
collections. For example, the `Lake/Reservoir Storage` group currently
maps USBR RISE parameter `3` to USACE storage parameters such as
...
`queryables` to inspect attributes and `items` to retrieve features. EDR
collections may also expose `/items`; in that case, treat `/items` as
the feature catalog you use to discover stations, reservoirs, dams, and
...
It contains USBR RISE lake/reservoir features such as Lake Havasu Parker
Dam and Powerplant (`3515`) and USACE features such as Alamo Dam
(`157145`).
