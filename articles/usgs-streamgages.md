...
what’s on offer, follow every advertised location page inside a small
northern North Carolina Piedmont study area, pull recent streamflow for
each station, and render an interactive map. The displayed results were
precomputed from the live USGS endpoint so package checks remain
offline.
...
will fall back to per-station fetches instead of one bulk call. For a
handful of gauges that’s fine; for thousands it would be slow. The WWDH
`rise-edr` collection in
[`vignette("getting-started")`](https://ksonda.github.io/edr4r/articles/getting-started.md)
shows the bulk-fetch path against a richer server.

### Discovering parameters

[`edr_parameters()`](https://ksonda.github.io/edr4r/reference/edr_parameters.md)
reads the `parameter_names` block from the collection document – but
USGS doesn’t populate it. Use
[`edr_queryables()`](https://ksonda.github.io/edr4r/reference/edr_queryables.md)
instead to see what filter properties the server exposes:
...
A compact box around several Tar River tributaries keeps this example
small while still forcing more than one server page:
...
USGS paginates its location index with an opaque cursor. Opt into
bounded pagination to make “every gauge” explicit; `limit` is the page
size, while the two client-side caps guard the total pull. With
[`sf`](https://r-spatial.github.io/sf/) installed the combined response
is an `sf` object with one point per gauge:
...
a small ggplot wrapper for the tidy tibble:
...
![plot of chunk
plot-one-gauge](usgs-streamgages-figs/plot-one-gauge-1.png)

plot of chunk plot-one-gauge

If you already know the station IDs, the public batch helper performs
the same bounded per-station workflow without discovering or
...
[`edr_explore()`](https://ksonda.github.io/edr4r/reference/edr_explore.md)
does the per-station loop and hands the results to
[`edr_map()`](https://ksonda.github.io/edr4r/reference/edr_map.md) in
one call. Each marker becomes an inline plot + CSV download for that
gauge.
...
Print `m` in an interactive R session to open the map. Click any blue
marker to open the popup; its Jan–May 2026 streamflow plot is inline,
and the “Download CSV” link delivers that gauge’s series.
...
- Pass `label_col = "monitoring_location_name"` to
  [`edr_map()`](https://ksonda.github.io/edr4r/reference/edr_map.md) so
  popup titles read “BUFFALO CREEK NEAR FALLS, NC” instead of the raw
  `USGS-02087183` id.

## See also

- [`vignette("cross-endpoint-water-context")`](https://ksonda.github.io/edr4r/articles/cross-endpoint-water-context.md)
  – place a USGS Hoover Dam discharge series beside WWDH Lake Mead
  storage and a Met Office population grid.
- [`vignette("getting-started")`](https://ksonda.github.io/edr4r/articles/getting-started.md)
