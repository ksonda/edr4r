...
`edr4r` is a thin, tidy client for OGC API - EDR services. This vignette walks
through a typical session against the Western Water Datahub (WWDH).
...
The client just stores connection settings (base URL, user agent, timeout,
retry policy). Pass `verbose = TRUE` to echo every request URL, or `headers =`
...
With no filters, `locations` returns a GeoJSON `FeatureCollection`, which
`edr4r` promotes to an `sf` object (if `sf` is installed):
...
Add a `location_id` and a `datetime` to get CoverageJSON, then flatten it:
...
`cube` takes a bounding box; `area` takes a polygon (WKT, an lon/lat matrix,
or an `sf` polygon):
...
- `datetime` accepts `"start/end"`, open intervals (`"2020-01-01/.."`), or a
...
- WWDH implements `locations`, `cube`, and `area`. The `position`, `radius`,
  `trajectory`, and `corridor` verbs exist in this client for spec
  completeness and other servers, but will error against collections that do
  not implement them.
```
