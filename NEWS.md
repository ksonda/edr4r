...
* Update USGS examples and live smoke checks to the v1 API and its `edr/daily`
  collection. Collection IDs can contain slash-separated path segments;
  empty segments and `.` or `..` are rejected. Document v1 support for
  `datetime` intervals on individual location requests.
